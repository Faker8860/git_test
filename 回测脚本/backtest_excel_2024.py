"""2024年 Volty 策略回测 → Excel | 仓位: 50% × 3x | 起始: $700"""
...
print("=" * 60)
print("  2024年 Volty 策略回测 -> Excel")
print(f"  仓位: {PCT}% x {LEV}x | 起始: ${INIT}")
print("=" * 60)

# === Download ===
print(">>> Downloading 2024 ETHUSDT 15m...")
...
for i in range(START, len(df)):
    ...
    tm = df.index[i]; date_key = str(tm.date())
...
if pos:
    lp=df["close"].iloc[-1]
    pl=(lp-ep)/ep*uv if pos=="LONG" else (ep-lp)/ep*uv; bal+=pl
...
wins=closed[closed.iloc[:,8]>0]; losses=closed[closed.iloc[:,8]<0]
...
for w in sorted(trades_df.iloc[:,10].unique()):
    ...
    prev=closed[closed.iloc[:,10]<w]; ws=prev.iloc[-1,9] if not prev.empty else INIT
    we=wt.iloc[-1,9]; wr=round((we-ws)/ws*100,2) if ws>0 else 0
...
for m in range(1,13):
    ...
    if mt.empty: monthly_data.append([f"{m}月",0,0,0,0,0,0,0]); continue
    mpnl=round(mt.iloc[:,8].sum(),2)
    prev=closed[closed.iloc[:,11]<m]; ms=prev.iloc[-1,9] if not prev.empty else INIT
    me=mt.iloc[-1,9]; mr=round((me-ms)/ms*100,2) if ms>0 else 0
...
def sh(ws,row,n):
    for c in range(1,n+1): cl=ws.cell(row=row,column=c); cl.font=hf; cl.fill=hfill; cl.alignment=Alignment(horizontal="center",vertical="center"); cl.border=tb

def sd(ws,row,n,even=False):
    for c in range(1,n+1): cl=ws.cell(row=row,column=c); cl.font=df_; cl.border=tb; cl.alignment=Alignment(horizontal="center" if c>2 else "left")
    if even:
        for c in range(1,n+1): ws.cell(row=row,column=c).fill=lfill

# Sheet 1
ws1=wb.active; ws1.title="Transaction Details"
ws1.merge_cells("A1:L1"); ws1.cell(row=1,column=1,value="2024 Volty Expan Close Strategy - Trade Details").font=tf
ws1.merge_cells("A2:L2"); ws1.cell(row=2,column=1,value=f"Position: {PCT}% x {LEV}x | Start: ${INIT} | End: ${final_bal:,.0f} | Return: {total_ret:+.1f}%").font=sf
...
ws2.merge_cells("A1:H1"); ws2.cell(row=1,column=1,value="2024 Monthly PnL Summary").font=tf
...
ws3.merge_cells("A1:H1"); ws3.cell(row=1,column=1,value="2024 Weekly PnL Summary").font=tf
...
ws4.merge_cells("A1:B1"); ws4.cell(row=1,column=1,value="2024 Strategy Statistics").font=tf
stats=[
    ("Strategy","Volty Expan Close"),("Symbol","ETHUSDT Perpetual"),("Timeframe","15min"),
    ("Params",f"VLEN={VLEN}, MULT={VMULT}"),("Trend Filter","EMA50"),
    ("Position",f"{PCT}% x {LEV}x"),("Stop Loss",f"{SL_PCT}%"),("",""),
    ("Start Balance",f"${INIT:,.2f}"),("End Balance",f"${final_bal:,.2f}"),
    ("Total PnL",f"${final_bal-INIT:+,.2f}"),("Total Return",f"{total_ret:+.2f}%"),("",""),
    ("Total Trades",len(trades_df)),("Closed Trades",len(closed)),
    ...
    ("Max Drawdown",f"{max_dd:+.2f}%"),("Max DD Date",max_dd_date),
    ("Profitable Months",f"{len([m for m in monthly_data if m[5]>0])}/12"),
...
# Save
out = "/opt/trading/2024_Volty_Backtest_50pct_3x.xlsx"
od = os.path.dirname(out)
if od: os.makedirs(od, exist_ok=True)
wb.save(out)
print(f"\nExcel: {out}")
print(f"Final: ${INIT:,} -> ${final_bal:,.0f} ({total_ret:+.1f}%)")
print(f"{len(trades_df)} trades, {len(closed)} closed, {len(wins)}W/{len(losses)}L")
